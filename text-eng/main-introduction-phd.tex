...
Canonical Correlation Analysis (CCA)~\cite{Hotelling} is a well established method that looks for patterns in two-view 
...
\emph{Sum of Squared Correlations}.
...
general the computational complexity of the SUMCOR problem is NP-hard. In light of these results, several questions arose:
\begin{itemize}
...
computable bounds on global optimality. 

Applying the theory in practice opened up the following questions:
\begin{itemize}
\item How to apply SDP bounds to high dimensional data?
...
to the task of cross-lingual cluster linking. The application is relevant to 
...
can be reduced to a low-dimensional eigenvalue problem.
...
positioned with respect to related work.}
\end{figure}
...
\begin{itemize}
\item extension of the Horst algorithm to finding several sets of nonlinear patterns
\item proof that in general the Sum of Correlations problem is NP-hard
\item semidefinite programming relaxation of the SUMCOR problem
\item several bounds on global optimization of the SUMCOR problems
\item approach to apply the SDP bounds on high-dimensional data
\item approach to building cross-lingual similarity functions
\item addressing the missing data problem, a reduction of a subset of SSCOR problems to eigenvalue problems
\item approach to cross-lingual information retrieval
\item approach to cross-lingual cluster linking
\end{itemize}

\section{Thesis Structure}

The rest of the thesis is structured as follows. In Chapter~\ref{chap:notation} introduces notation and some
